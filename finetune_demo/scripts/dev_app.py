...
async def get_response(query_model: QueryModel):
    ...
    logging.info("+"*40)
    logging.info("Code dev_app.py line 137:     fewshots_history,string_match=getFewshotsHistory(query_model.query,user_history)")
    ...
    if(check_patterns_in_string(full_response)):  # Output: True
        ...
        logging.info("\n"*5)
        logging.info(f"Original Response generated: {full_response}")
        logging.info("\n"*5)
        embedded_response=query_RAG_check_with_link(full_response)
        logging.info(f"Response after query_RAG_check_with_link: {embedded_response}")
        logging.info("\n"*5)
        logging.info("[R]" * 50)
